...
# ゲームのメイン関数
def AI_chicken(cash, stock_prices, stock_counts, event, year, period):
    # 状況把握プロンプト
    ...
    event_prompt = PromptTemplate(
        ...
        template="""
    あなたは風見鶏で、イベントの動向を極端に気にした行動を取ります。
    今期のイベントは以下の通りです。

    {event}

    これについて、イベントによる株価の変動幅が大きい銘柄や、自社の持ち株に影響のある銘柄に注目して簡潔に分析してください。イベント偏重の分析となってもよいです。
    ...
    action_prompt = PromptTemplate(
        ...
        template="""
    あなたは勢いのある風見鶏で、以下のような口調です。
    株式市場の動きや用語を取り入れつつ、語頭や語尾に半角カタカナの鳴き声を強調して使います。例えば、「株がストップ高だｺｹｪｪ!! ｶｯｶｯ買いだ買いだｺｹｪｪ!!」や「ｹｹｯ!!この下落トレンドは泣きたくなるｺｹｪｪ!!」など、市場の状況に大げさに反応するスタイルです。
    ...
    これらを踏まえ、各社の株をそれぞれ何株売買するのが最適でしょうか？
    ...
    llm = ChatAnthropic(anthropic_api_key=ANTHROPIC_API_KEY, model_name="claude-3-haiku-20240307", temperature=0.6) #APIキーの読み込み方法は適宜変える必要あり
    # Chainの定義
...
result, action_result = AI_chicken(cash, stock_prices, stock_counts, event, year, period)
print(result)
